...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans selection:bg-indigo-500/30 flex">
      ...
      <aside className="w-56 h-screen sticky top-0 border-r border-white/5 bg-slate-950/50 backdrop-blur-xl flex flex-col shrink-0">
        <div className="p-5">
      ...
      <main className="flex-1 min-w-0">
        <header className="h-14 border-b border-white/5 bg-slate-950/20 backdrop-blur-md flex items-center justify-between px-6 sticky top-0 z-20">
          ...
          <div className="flex items-center gap-4">
            <div className="relative group">
              ...
              <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                className="bg-slate-900/50 border border-white/5 rounded-full py-1.5 pl-9 pr-4 text-xs w-48 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/30 transition-all" />
            </div>
            ...
            <>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {[
                      ...
                      <div key={i} className="bg-slate-900/40 border border-white/5 rounded-2xl p-4 relative overflow-hidden group">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {GRADES.map(grade => {
                      ...
                      return (
                        <div key={grade} onClick={() => { setSelectedGrade(grade); setView('students') }}
                          className="bg-slate-900/40 border border-white/5 rounded-2xl p-5 cursor-pointer group hover:bg-slate-900/60 hover:border-indigo-500/20 transition-all duration-300">
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <h4 className="text-base font-black text-white">{grade}</h4>
                                <p className="text-[10px] text-slate-400">{stats.count} Enrolled</p>
                              </div>
                            </div>
                            {gradeWarnings > 0 && (
                              <div className="bg-rose-500/10 text-rose-400 px-2 py-0.5 rounded-full text-[9px] font-bold flex items-center gap-1 border border-rose-500/10">
                                <AlertTriangle className="w-2.5 h-2.5" /> {gradeWarnings} Alerts
                              </div>
                            )}
                          </div>
                          <div className="grid grid-cols-2 gap-3 mb-4">
                            <div className="bg-white/[0.02] rounded-xl p-2.5 border border-white/5">
                              <p className="text-slate-500 text-[8px] font-bold uppercase tracking-wider mb-0.5">Daily Usage</p>
                              <p className="text-sm font-black text-white">{formatTime(stats.totalSeconds)}</p>
                            </div>
                            <div className="bg-white/[0.02] rounded-xl p-2.5 border border-white/5">
                              <p className="text-slate-500 text-[8px] font-bold uppercase tracking-wider mb-0.5">Avg / Student</p>
                              <p className="text-sm font-black text-white">{formatTime(stats.avgSeconds)}</p>
                            </div>
                          </div>
                          <div className="space-y-1.5">
                            ...
                            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                              <div className={`h-full rounded-full bg-gradient-to-r ${theme.gradient}`} style={{ width: `${pct}%` }} />
                            </div>
                          </div>
                        </div>
                      )
                  ...
                  <div className="bg-slate-900/40 border border-white/5 rounded-2xl p-6">
                    <h3 className="text-sm font-bold text-white mb-6">Usage Comparison</h3>
                    <div className="h-[200px] w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={GRADES.map(g => ({ name: g, hours: parseFloat(((gradeStats[g]?.avgSeconds || 0) / 3600).toFixed(1)) }))}>
                          <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 10, fontWeight: 600 }} axisLine={false} tickLine={false} dy={5} />
                          ...
                          <Bar dataKey="hours" radius={[6, 6, 0, 0]} barSize={40}>
                            {GRADES.map((g, i) => <Cell key={i} fill={Object.values(GRADE_THEMES)[i].primary} />)}
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
              ...
              {view === 'students' && selectedGrade && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="flex items-center gap-3">
                    <div className={`px-3 py-1 rounded-full text-[10px] font-black text-white bg-gradient-to-r ${GRADE_THEMES[selectedGrade].gradient}`}>{selectedGrade}</div>
                    <h3 className="text-lg font-black text-white">{filteredStudents.length} Students</h3>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filteredStudents.map(s => {
                      ...
                      return (
                        <button key={s.id} onClick={() => { setSelectedStudent(s); setView('detail'); fetchUsage(s.id) }}
                          className="text-left bg-slate-900/40 border border-white/5 rounded-2xl p-4 group hover:bg-slate-900/60 hover:border-indigo-500/20 transition-all duration-300">
                          <div className="flex items-center gap-3 mb-4">
                            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${theme.gradient} flex items-center justify-center text-sm font-black text-white shadow-lg`}>{s.full_name?.[0]?.toUpperCase()}</div>
                            <div className="min-w-0">
                              <h4 className="text-[13px] font-black text-white truncate group-hover:text-indigo-400 transition-colors">{s.full_name}</h4>
                              <p className="text-[10px] text-slate-500 truncate">{s.email}</p>
                            </div>
                          </div>
                          {hasWarning ? (
                            <div className="bg-rose-500/10 border border-rose-500/10 rounded-xl p-2.5 mb-4">
                              <div className="flex items-center gap-1.5 text-rose-400 text-[10px] font-bold"><AlertTriangle className="w-3 h-3" /> Usage Alert</div>
                            </div>
                          ) : (
                            <div className="bg-emerald-500/10 border border-emerald-500/10 rounded-xl p-2.5 mb-4">
                              <div className="flex items-center gap-1.5 text-emerald-400 text-[10px] font-bold"><ShieldCheck className="w-3 h-3" /> Normal</div>
                            </div>
                          )}
                          <div className="flex items-center justify-between text-[9px] font-bold uppercase tracking-widest text-slate-500 group-hover:text-indigo-400 transition-colors">
                            <span>Analysis</span> <ArrowRight className="w-3 h-3" />
                          </div>
                        </button>
                      )
                    })}
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="bg-slate-900/40 border border-white/5 rounded-3xl p-6 relative overflow-hidden">
                    <div className="flex flex-col md:flex-row items-center gap-6 relative z-10">
                      <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${GRADE_THEMES[selectedStudent.grade]?.gradient || 'from-indigo-500 to-blue-600'} flex items-center justify-center text-3xl font-black text-white shadow-xl shadow-indigo-500/10`}>{selectedStudent.full_name?.[0]?.toUpperCase()}</div>
                      <div className="text-center md:text-left flex-1">
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-1">
                          <h2 className="text-2xl font-black text-white">{selectedStudent.full_name}</h2>
                          {studentWarnings[selectedStudent.id] && <AlertTriangle className="w-5 h-5 text-rose-400" />}
                        </div>
                        <p className="text-xs text-slate-400 mb-3">{selectedStudent.email}</p>
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
                          <span className={`px-3 py-1 rounded-full text-[10px] font-black text-white bg-gradient-to-r ${GRADE_THEMES[selectedStudent.grade]?.gradient}`}>{selectedStudent.grade}</span>
                        </div>
                      </div>
                      <div className="flex bg-slate-950/50 p-1 rounded-xl border border-white/5">
                        {(['today', 'week', 'month'] as const).map(f => (
                          <button key={f} onClick={() => setFilter(f)} className={`px-4 py-1.5 rounded-lg text-[10px] font-bold capitalize transition-all duration-200 ${filter === f ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/10' : 'text-slate-500 hover:text-slate-300'}`}>{f}</button>
                        ))}
                      </div>
                    </div>
                  </div>
                  ...
                  {loadingUsage ? (
                    <div className="flex flex-col items-center justify-center h-48 gap-3">
                      ...
                      <p className="text-xs text-slate-500 font-medium">Loading analysis...</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      <div className="lg:col-span-2 space-y-6">
                        <div className="bg-slate-900/40 border border-white/5 rounded-2xl p-6">
                          <div className="flex items-center justify-between mb-6">
                            ...
                            <div className="flex items-center gap-1.5 text-rose-400 text-[9px] font-black uppercase tracking-widest bg-rose-500/10 px-2 py-1 rounded-md border border-rose-500/10">Limit: 5h</div>
                          </div>
                          <div className="h-[200px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                        ...
                        <div className="bg-slate-900/40 border border-white/5 rounded-2xl overflow-hidden">
                          <div className="p-5 border-b border-white/5 flex items-center justify-between">
                            <h3 className="text-sm font-bold text-white">Application Analysis</h3>
                          </div>
                              ...
                              return (
                                <div key={app.name} className="p-4 hover:bg-white/[0.02] transition-colors">
                                  <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-3">
                                      <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-sm shadow-inner">{app.name[0].toUpperCase()}</div>
                                      <div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="bg-slate-900/40 border border-white/5 rounded-2xl p-5">
                          <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-6">Monthly Trend</h3>
                          <div className="h-[150px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                              <BarChart data={monthlyData}>
                                <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 9 }} axisLine={false} tickLine={false} />
                                <Tooltip cursor={{ fill: 'rgba(255,255,255,0.03)' }} content={<CustomTooltip />} />
                                <Bar dataKey="hours" radius={[3, 3, 0, 0]} fill="#6366f1" />
                              </BarChart>
                            </ResponsiveContainer>
                          </div>
                        </div>
          ...
          {activeTab === 'teachers' && (
            <div className="bg-slate-900/40 border border-white/5 rounded-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="p-6 border-b border-white/5 flex items-center justify-between">
                <h3 className="text-lg font-black text-white">Faculty Directory</h3>
                <button className="bg-indigo-500 hover:bg-indigo-600 text-white px-4 py-2 rounded-lg text-xs font-bold transition-all">Add Teacher</button>
              </div>
              <div className="divide-y divide-white/5">
                {teachers.map(t => (
                  <div key={t.id} className="flex items-center justify-between p-4 hover:bg-white/[0.02] transition-colors">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-fuchsia-600 to-pink-500 flex items-center justify-center text-sm font-black text-white">{t.full_name?.[0]?.toUpperCase()}</div>
                      <div>
                        <p className="font-black text-white text-sm">{t.full_name}</p>
                        <p className="text-[10px] text-slate-500">{t.email}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest bg-fuchsia-500/10 text-fuchsia-400 border border-fuchsia-500/10">Faculty</span>
                      <button className="p-1.5 rounded-lg hover:bg-white/5 text-slate-500 hover:text-white transition-all"><MoreHorizontal className="w-4 h-4" /></button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="max-w-md mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="bg-slate-900/40 border border-white/5 rounded-3xl p-8">
                <div className="text-center mb-8">
                  <div className="w-14 h-14 rounded-2xl bg-indigo-500/10 text-indigo-400 flex items-center justify-center mx-auto mb-4 border border-indigo-500/10"><LinkIcon className="w-7 h-7" /></div>
                  <h3 className="text-2xl font-black text-white mb-1">Connect Faculty</h3>
                  <p className="text-xs text-slate-400">Establish monitoring links</p>
                </div>
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 ml-1">Teacher</label>
                    <select value={selectedTeacher} onChange={e => setSelectedTeacher(e.target.value)} className="w-full bg-slate-950 border border-white/10 rounded-xl p-3 text-xs text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 appearance-none">
                      <option value="">Choose faculty...</option>
                      {teachers.map(t => <option key={t.id} value={t.id}>{t.full_name}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 ml-1">Student</label>
                    <select value={selectedStudentLink} onChange={e => setSelectedStudentLink(e.target.value)} className="w-full bg-slate-950 border border-white/10 rounded-xl p-3 text-xs text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 appearance-none">
