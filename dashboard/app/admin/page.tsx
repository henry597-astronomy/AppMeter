            ...
            {activeTab === 'grades' && (
              <>
                ...
                {view === 'detail' && selectedStudent && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="flex items-center gap-3 mb-4"><button onClick={() => setView('students')} className="p-1.5 rounded-lg bg-white/5 text-slate-400 hover:text-white"><ChevronLeft className="w-4 h-4" /></button><h3 className="text-sm font-bold text-white">Student Details - {selectedGrade}</h3></div>
                    <div className="bg-slate-900/40 border border-white/5 rounded-3xl p-6"><div className="flex flex-col sm:flex-row items-center gap-6"><div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${GRADE_THEMES[selectedStudent.grade]?.gradient || 'from-indigo-500 to-blue-600'} flex items-center justify-center text-3xl font-black text-white shadow-xl`}>{selectedStudent.full_name?.[0]?.toUpperCase()}</div><div className="text-center sm:text-left flex-1"><h2 className="text-2xl font-black text-white mb-2">{selectedStudent.full_name}</h2><div className="space-y-1 mb-3"><p className="text-xs text-slate-400">Email: {selectedStudent.email}</p><p className="text-xs text-slate-500">Student ID: <span className="font-mono text-slate-300">{selectedStudent.id?.slice(0, 8)}...</span></p></div><div className="flex flex-wrap gap-2"><span className={`px-3 py-1 rounded-full text-[10px] font-black text-white bg-gradient-to-r ${GRADE_THEMES[selectedStudent.grade]?.gradient}`}>{selectedStudent.grade}</span><span className="px-3 py-1 rounded-full text-[10px] font-black text-white bg-indigo-500/20 border border-indigo-500/10">Active</span></div></div><div className="flex bg-slate-950/50 p-1 rounded-xl border border-white/5">{(['today', 'week', 'month'] as const).map(f => (<button key={f} onClick={() => setFilter(f)} className={`px-3 sm:px-4 py-1.5 rounded-lg text-[10px] font-bold capitalize transition-all duration-200 ${filter === f ? 'bg-indigo-500 text-white' : 'text-slate-500 hover:text-slate-300'}`}>{f}</button>))}</div></div></div>
