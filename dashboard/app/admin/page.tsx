    ...
    <div className="p-5 flex flex-col h-full">
      ...
      <nav className="space-y-1">
        {[
          ...
          <button
            key={item.id}
            onClick={() => { setActiveTab(item.id as any); setView('grades'); setSelectedStudent(null); setSelectedGrade(null); setIsSidebarOpen(false) }}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-14 border-b border-white/5 bg-slate-950/20 backdrop-blur-md flex items-center justify-between px-4 lg:px-8 sticky top-0 z-20">
          <div className="flex items-center gap-3">
            <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-1.5 rounded-lg bg-white/5 text-slate-400">
            ...
            <h2 className="text-sm font-bold text-white capitalize">
              {activeTab === 'grades' ? (view === 'grades' ? 'Overview' : (view === 'students' ? `${selectedGrade}` : 'Detail')) : activeTab}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:block relative group">
              ...
              <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                className="bg-slate-900/50 border border-white/5 rounded-full py-1.5 pl-9 pr-4 text-xs w-32 md:w-48 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/30 transition-all" />
            </div>
            <div className="flex items-center gap-2 sm:pl-4 sm:border-l sm:border-white/5">
                    ...
                    {filteredStudents.map(s => {
                      ...
                      return (
                        <button key={s.id} onClick={() => { setSelectedStudent(s); setView('detail'); fetchUsage(s.id) }}
                          className="text-left bg-slate-900/40 border border-white/5 rounded-2xl p-5 group hover:bg-slate-900/60 hover:border-indigo-500/20 transition-all duration-300">
                          <div className="flex items-center gap-3 mb-4">
                            ...
                            <div className="min-w-0"><h4 className="text-[14px] font-black text-white truncate group-hover:text-indigo-400 transition-colors">{s.full_name}</h4><p className="text-[10px] text-slate-500 truncate">{s.email}</p></div>
                          </div>
                          <div className={`rounded-xl p-2.5 mb-4 text-[10px] font-bold flex items-center gap-1.5 ${hasWarning ? 'bg-rose-500/10 text-rose-400 border border-rose-500/10' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/10'}`}>
                            {hasWarning ? <><AlertTriangle className="w-3.5 h-3.5" /> Warning</> : <><ShieldCheck className="w-3.5 h-3.5" /> Normal</>}
                          </div>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="bg-slate-900/40 border border-white/5 rounded-3xl p-6">
                    <div className="flex flex-col sm:flex-row items-center gap-6">
                      <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${GRADE_THEMES[selectedStudent.grade]?.gradient || 'from-indigo-500 to-blue-600'} flex items-center justify-center text-3xl font-black text-white shadow-xl`}>{selectedStudent.full_name?.[0]?.toUpperCase()}</div>
                      <div className="text-center sm:text-left flex-1">
                        <h2 className="text-2xl font-black text-white mb-1">{selectedStudent.full_name}</h2>
                      ...
                      <div className="flex bg-slate-950/50 p-1 rounded-xl border border-white/5">
                        {(['today', 'week', 'month'] as const).map(f => (
                          <button key={f} onClick={() => setFilter(f)} className={`px-3 sm:px-4 py-1.5 rounded-lg text-[10px] font-bold capitalize transition-all duration-200 ${filter === f ? 'bg-indigo-500 text-white' : 'text-slate-500 hover:text-slate-300'}`}>{f}</button>
                        ))}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-6">
                      <div className="bg-slate-900/40 border border-white/5 rounded-2xl p-6 overflow-hidden">
                        <h3 className="text-sm font-bold text-white mb-6">Activity Timeline</h3>
                        <div className="h-[200px] w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={dailyData}><XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis tick={{ fill: '#64748b', fontSize: 10 }} axisLine={false} tickLine={false} /><Bar dataKey="hours" radius={[4, 4, 0, 0]} barSize={30}>{dailyData.map((d, i) => <Cell key={i} fill={d.warning ? '#f43f5e' : '#6366f1'} />)}</Bar></BarChart></ResponsiveContainer></div>
                      </div>
                      <div className="bg-slate-900/40 border border-white/5 rounded-2xl overflow-hidden">
                        <div className="p-5 border-b border-white/5"><h3 className="text-sm font-bold text-white">App Breakdown</h3></div>
                          ...
                          {usage.map((app, i) => {
                            ...
                            return (
                              <div key={app.name} className="p-4 hover:bg-white/[0.02] transition-colors">
                                <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-sm shadow-inner">{app.name[0].toUpperCase()}</div><div><p className="text-[13px] font-bold text-white">{app.name}</p><p className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">{pct}% of total</p></div></div><p className="text-sm font-black text-white">{formatTime(app.seconds)}</p></div>
                                <div className="h-1 bg-slate-800 rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-1000" style={{ width: `${pct}%`, backgroundColor: COLORS[i % COLORS.length] }} /></div>
                              </div>
                            )
                          })}
                        </div>
